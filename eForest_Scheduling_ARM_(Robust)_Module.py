...
import random, os, arl_utils, sys
import pandas as pd
...
def support_finder(df, ColumnTarget, SupportMethod): #This method generates a number for the support attribute of the
...
def dataframe_appender(dfList, howMany): #This method concatenates dataframes from the dfs list to get a more robust
...
def apyori_robust_rule_finder (dfList, howMany, robustness, supportColumnTarget, supportMethod ):
    ...
    return association_results                              #aka rule to be made.

rules = apyori_robust_rule_finder(dfs, 8, 12, 'Room', 'Lower IQR') #Function call entails everything to find rules.
